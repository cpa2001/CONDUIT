...
class Qwen2_5_VisionTransformer(nn.Module, RotaryPosMixin):
    ...
    @record_function("[Qwen2_5_VisionTransformer] forward")
    def forward(
        self,
        x: torch.Tensor,
...
def build_qwen2_5_vl_position_ids(
    ...
    for image_index in range(int(image_grid_thw.shape[0])):
        try:
            image_start = input_tokens.index(image_token_id, start)
        except ValueError as exc:
            raise ValueError("Failed to locate image placeholder span in input_ids.") from exc

        temporal, height, width = [int(value) for value in image_grid_thw[image_index].tolist()]
        llm_grid_t = temporal
        llm_grid_h = height // spatial_merge_size
        llm_grid_w = width // spatial_merge_size
        image_span = llm_grid_t * llm_grid_h * llm_grid_w
        text_len = image_start - start
        start_index = llm_pos_ids_list[-1].max().item() + 1 if llm_pos_ids_list else 0

        if text_len > 0:
            text_positions = torch.arange(text_len, dtype=torch.long, device="cpu")
            llm_pos_ids_list.append(
                text_positions.view(1, -1).expand(3, -1) + start_index
            )

        t_index = torch.zeros(image_span, dtype=torch.long, device="cpu")
        h_index = torch.arange(llm_grid_h, dtype=torch.long, device="cpu").view(1, -1, 1)
        h_index = h_index.expand(llm_grid_t, -1, llm_grid_w).flatten()
        w_index = torch.arange(llm_grid_w, dtype=torch.long, device="cpu").view(1, 1, -1)
        w_index = w_index.expand(llm_grid_t, llm_grid_h, -1).flatten()
        llm_pos_ids_list.append(
            torch.stack([t_index, h_index, w_index]) + text_len + start_index
        )
        start = image_start + image_span
    ...
    positions = torch.cat(llm_pos_ids_list, dim=1)
    if positions.shape[1] != len(input_tokens):
        raise ValueError(
            f"Multimodal position count mismatch: {positions.shape[1]} vs {len(input_tokens)}."
        )
    position_offset = int(positions.max().item() + 1 - len(input_tokens))
...
class Qwen2_5_VLForConditionalGeneration(nn.Module):
    ...
    def get_visual_features(
        ...
        outputs = self.visual(pixel_values, grid_thw)
        ...
        if isinstance(outputs, (tuple, list)) and outputs:
            return outputs[0]
        raise TypeError(f"Unsupported Qwen2.5-VL visual output type: {type(outputs)!r}")
    ...
    def get_input_positions(
        ...
        return build_qwen2_5_vl_position_ids(
            input_ids=input_ids,
    ...
    @record_function("[Qwen2_5_VLForConditionalGeneration] forward")
    def forward(
        self,
        ...
        if visual_embeds is not None:
            ...
            assert (
                image_mask.sum() == visual_embeds.shape[0]
